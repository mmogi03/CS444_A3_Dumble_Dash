...
export function castSpell(spellName) {
  ...
  if (spellName === "fireball") {
    const sfx = document.getElementById("audio-tail-whip");
    if (sfx) {
      sfx.currentTime = 0;
      sfx.play();
    }
  } else if (spellName === "mudWall") {
    const sfx = document.getElementById("audio-tidal-splash");
    if (sfx) {
      sfx.currentTime = 0;
      sfx.play();
    }
  } else if (spellName === "astralHeal") {
    const sfx = document.getElementById("audio-coral-heal");
    if (sfx) {
      sfx.currentTime = 0;
      sfx.play();
    }
  ...
  // Deduct mana and hide battle UI
...
export function showGameOver() {
  // Hide the game screen and show only the end-game screen
